...
!   06/24/2024 Satoki Tsujino    added the entry for get_nobs_H08uv
!
!=======================================================================
!
...
!$USE OMP_LIB
  ...
  INTEGER,PARAMETER :: nid_obs_varlocal=13 !H08 and H08vt/vr/uv
...
!
  ...
  INTEGER,PARAMETER :: id_h08ux_obs=8902  ! by satoki
  INTEGER,PARAMETER :: id_h08vy_obs=8903  ! by satoki
...
!ORG(satoki)     (/'WND', '  T', 'MOI', ' PS', 'PRC', 'TCV', 'REF', ' Vr', 'H08'/)
  ...
  CHARACTER(6),PARAMETER :: obtypelist(nobtype)= &
     (/'ADPUPA', 'AIRCAR', 'AIRCFT', 'SATWND', 'PROFLR', &
       ...
       'TMPAPR', 'PHARAD', 'H08IRB', 'TCVITL', 'H08VTA', &
       'H08VRA', 'H08UXA', 'H08VYA'/) ! H08
!ORG(satoki)       'TMPAPR', 'PHARAD', 'H08IRB', 'TCVITL'/) ! H08
  ...
  character(obsformatlenmax), parameter :: obsfmt_h08ux    = 'H08UX'  ! add by satoki
  character(obsformatlenmax), parameter :: obsfmt_h08vy    = 'H08VY'  ! add by satoki
...
function uid_obs(id_obs)
  ...
  case(id_h08vr_obs)  ! H08VR (adding by satoki)
    uid_obs = 18
  case(id_h08ux_obs)  ! H08UV (adding by satoki)
    uid_obs = 19
  case(id_h08vy_obs)  ! H08UV (adding by satoki)
    uid_obs = 20
  case default
    uid_obs = -1     ! error
  end select
...
!-----------------------------------------------------------------------
SUBROUTINE vert_ave( kz, z1, z2, zval, ival, oval )
  IMPLICIT NONE
...
!-----------------------------------------------------------------------
SUBROUTINE azim_sum( nt, azim_val, i_counter, sum_val, ic )
  IMPLICIT NONE
  ...
  INTEGER, INTENT(in) :: i_counter(nt)       ! flag for candidate sampling point (1 = OK, 0 = NG)
  REAL(r_size), INTENT(out) :: sum_val       ! sum of azim_val in azimuthal direction
  REAL(r_size), INTENT(out), OPTIONAL :: ic  ! sampling number for averaging (i.e., sum of i_counter)
  INTEGER :: ii
  REAL(r_size) :: nc
...
SUBROUTINE rgrt_floor( ni, nj, ri_ref, rj_ref, ri, rj, i_ri, j_rj, floor_ri, floor_rj )
  ...
  INTEGER, INTENT(out) :: i_ri,j_rj              ! element numbers for floor_ri,floor_rj
...
!!!!!!#endif
    ...
    if (DEPARTURE_STAT_T_RANGE <= 0.0d0 .or. &
      ...
      !-------------------------------------------------------------------------
         write(*,*) "Under construction by satoki"
      !=========================================================================
      case (obsfmt_h08ux,obsfmt_h08vy)
      !-------------------------------------------------------------------------
         write(*,*) "Under construction by satoki"
#ifdef H08
      !=========================================================================
...
subroutine read_obs_all(obs)
  ...
  do iof = 1, OBS_IN_NUM
    ...
    case (obsfmt_h08ux,obsfmt_h08vy)
      call get_nobs_H08uv(trim(OBS_IN_NAME(iof)),obs(iof)%nobs) ! H08uv (by satoki)
    case default
    ...
    case (obsfmt_h08ux,obsfmt_h08vy)  ! Add by satoki
      call read_obs_H08uv(trim(OBS_IN_NAME(iof)),obs(iof)) ! H08uv
    end select
...
subroutine write_obs_all(obs, missing, file_suffix)
  ...
  do iof = 1, OBS_IN_NUM
    ...
    case (obsfmt_h08ux,obsfmt_h08vy)  ! Added by satoki
      call write_obs_H08uv(trim(filestr),obs(iof),missing=missing_) ! H08uv
    end select
  end do ! [ iof = 1, OBS_IN_NUM ]
...
!-- Read observation number for H08uv
SUBROUTINE get_nobs_H08uv(cfile,nn)
  IMPLICIT NONE
  CHARACTER(*),INTENT(IN) :: cfile
  INTEGER,INTENT(OUT) :: nn ! num of all H08uv obs
  INTEGER :: iprof
  INTEGER :: iunit
  LOGICAL :: ex
  INTEGER :: sz
...
!-----------------------------
  ...
  RETURN
END SUBROUTINE get_nobs_H08uv

!-- Read observation data for H08uv
SUBROUTINE read_obs_H08uv(cfile,obs)
  IMPLICIT NONE
  CHARACTER(*),INTENT(IN) :: cfile
  TYPE(obs_info),INTENT(INOUT) :: obs
  REAL(r_sngl) :: wk(8)
!  REAL(r_sngl) :: tmp
  ...
  DO n=1,nprof
    ...
    obs%typ(n) = NINT(wk(7))
    obs%dif(n) = REAL(wk(8),r_size)
write(*,*) "read_obs_H08uv check", n, obs%elm(n), obs%lon(n), obs%lat(n),  &
  &        obs%lev(n), obs%dat(n), obs%err(n)

  END DO
  CLOSE(iunit)

  RETURN
END SUBROUTINE read_obs_H08uv

!-- Write observation data for H08uv
SUBROUTINE write_obs_H08uv(cfile,obs,append,missing)
  IMPLICIT NONE
  CHARACTER(*),INTENT(IN) :: cfile
  TYPE(obs_info),INTENT(IN) :: obs
  LOGICAL,INTENT(IN),OPTIONAL :: append
  ...
  DO n=1,iprof
    ...
    wk(2) = REAL(obs%lon(n),r_size)
    wk(3) = REAL(obs%lat(n),r_size)
    wk(4) = REAL(obs%lev(n),r_size)
    wk(5) = REAL(obs%dat(n),r_size)
    wk(6) = REAL(obs%err(n),r_size)
    wk(7) = REAL(obs%typ(n))
    wk(8) = REAL(obs%dif(n),r_size)
    WRITE(iunit) wk
  ...
  RETURN
END SUBROUTINE write_obs_H08uv
