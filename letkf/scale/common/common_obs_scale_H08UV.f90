MODULE common_obs_scale_H08UV
!=======================================================================
!
! [PURPOSE:] Observational procedures for H08UV
!
! [HISTORY:]
...
!
!-----------------------------------------------------------------------
!   Himawari-8 UV obs subroutines by Satoki Tsujino (06/24/2024)
!   [Under construction]
!   Procedures:
!   Only spacial (linear) interpolation
!
!  [NOTE] stggrd: grid type of u and v  (Now, not supported)
!         0: non-staggered grid
!         1: staggered grid
!-----------------------------------------------------------------------
SUBROUTINE Trans_XtoY_H08UV(slot_v,procd_v,rig_tcobs,rjg_tcobs,rz,  &
  &                         lon,lat,valid,rigu,rigv,rjgu,rjgv,  &
  ...
  REAL(r_size),INTENT(IN) :: rz(slot_v,procd_v)  ! the level (unit: m) for the vertical average
...
!       end if
  ...
  RETURN
END SUBROUTINE Trans_XtoY_H08UV

END MODULE common_obs_scale_H08UV
