...
module mod_admin_restart
  ...
  character(len=H_LONG),  public :: RESTART_OUT_ADDITIONAL_BASENAME(3) = (/'','',''/)      !< Basename of the output file (add by satoki)
  integer,                public :: RESTART_OUT_ADDITIONAL_COPIES = 0         !< Basename of the output file (add by satoki)
...
!-- Add by satoki
  ...
  subroutine ADMIN_restart_write
    ...
    use mod_atmos_vars, only: &
       ATMOS_RESTART_OUTPUT,         &
       ATMOS_vars_restart_create_add,    &  ! add by satoki
    ...
    use mod_ocean_vars, only: &
       OCEAN_RESTART_OUTPUT,         &
       OCEAN_vars_restart_create_add,    &  ! add by satoki
    ...
    use mod_land_vars, only: &
       LAND_RESTART_OUTPUT,         &
       LAND_vars_restart_create_add,    &  ! add by satoki
    ...
    use mod_urban_vars, only: &
       URBAN_RESTART_OUTPUT,         &
       URBAN_vars_restart_create_add,    &  ! add by satoki
    ...
    !-- add by satoki for letkf from here (here will be rearranged by do loop in future)
    ! cread restart netCDF file(s)
    if( OCEAN_RESTART_OUTPUT .AND. TIME_DOOCEAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call OCEAN_vars_restart_create_add( RESTART_OUT_ADDITIONAL_BASENAME(1) )
    if(  LAND_RESTART_OUTPUT .AND. TIME_DOLAND_restart  .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call  LAND_vars_restart_create_add( RESTART_OUT_ADDITIONAL_BASENAME(1) )
    if( URBAN_RESTART_OUTPUT .AND. TIME_DOURBAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call URBAN_vars_restart_create_add( RESTART_OUT_ADDITIONAL_BASENAME(1) )
    if( ATMOS_RESTART_OUTPUT .AND. TIME_DOATMOS_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call ATMOS_vars_restart_create_add( RESTART_OUT_ADDITIONAL_BASENAME(1) )

    ! define metadata (dimensions, variables, attributes) in netCDF file
    if( OCEAN_RESTART_OUTPUT .AND. TIME_DOOCEAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call OCEAN_vars_restart_def_var
    if(  LAND_RESTART_OUTPUT .AND. TIME_DOLAND_restart  .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call  LAND_vars_restart_def_var
    if( URBAN_RESTART_OUTPUT .AND. TIME_DOURBAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call URBAN_vars_restart_def_var
    if( ATMOS_RESTART_OUTPUT .AND. TIME_DOATMOS_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call ATMOS_vars_restart_def_var

    ! exit define mode
    if( OCEAN_RESTART_OUTPUT .AND. TIME_DOOCEAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call OCEAN_vars_restart_enddef
    if(  LAND_RESTART_OUTPUT .AND. TIME_DOLAND_restart  .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call  LAND_vars_restart_enddef
    if( URBAN_RESTART_OUTPUT .AND. TIME_DOURBAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call URBAN_vars_restart_enddef
    if( ATMOS_RESTART_OUTPUT .AND. TIME_DOATMOS_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call ATMOS_vars_restart_enddef

    ! write variabes to netCDF file
    if( OCEAN_RESTART_OUTPUT .AND. TIME_DOOCEAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call OCEAN_vars_restart_write
    if(  LAND_RESTART_OUTPUT .AND. TIME_DOLAND_restart  .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call  LAND_vars_restart_write
    if( URBAN_RESTART_OUTPUT .AND. TIME_DOURBAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call URBAN_vars_restart_write
    if( ATMOS_RESTART_OUTPUT .AND. TIME_DOATMOS_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call ATMOS_vars_restart_write

    ! clode the restart file
    if( OCEAN_RESTART_OUTPUT .AND. TIME_DOOCEAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call OCEAN_vars_restart_close
    if(  LAND_RESTART_OUTPUT .AND. TIME_DOLAND_restart  .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call  LAND_vars_restart_close
    if( URBAN_RESTART_OUTPUT .AND. TIME_DOURBAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call URBAN_vars_restart_close
    if( ATMOS_RESTART_OUTPUT .AND. TIME_DOATMOS_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 0 ) call ATMOS_vars_restart_close

    ! cread restart netCDF file(s)
    if( OCEAN_RESTART_OUTPUT .AND. TIME_DOOCEAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call OCEAN_vars_restart_create_add( RESTART_OUT_ADDITIONAL_BASENAME(2) )
    if(  LAND_RESTART_OUTPUT .AND. TIME_DOLAND_restart  .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call  LAND_vars_restart_create_add( RESTART_OUT_ADDITIONAL_BASENAME(2) )
    if( URBAN_RESTART_OUTPUT .AND. TIME_DOURBAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call URBAN_vars_restart_create_add( RESTART_OUT_ADDITIONAL_BASENAME(2) )
    if( ATMOS_RESTART_OUTPUT .AND. TIME_DOATMOS_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call ATMOS_vars_restart_create_add( RESTART_OUT_ADDITIONAL_BASENAME(2) )

    ! define metadata (dimensions, variables, attributes) in netCDF file
    if( OCEAN_RESTART_OUTPUT .AND. TIME_DOOCEAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call OCEAN_vars_restart_def_var
    if(  LAND_RESTART_OUTPUT .AND. TIME_DOLAND_restart  .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call  LAND_vars_restart_def_var
    if( URBAN_RESTART_OUTPUT .AND. TIME_DOURBAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call URBAN_vars_restart_def_var
    if( ATMOS_RESTART_OUTPUT .AND. TIME_DOATMOS_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call ATMOS_vars_restart_def_var

    ! exit define mode
    if( OCEAN_RESTART_OUTPUT .AND. TIME_DOOCEAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call OCEAN_vars_restart_enddef
    if(  LAND_RESTART_OUTPUT .AND. TIME_DOLAND_restart  .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call  LAND_vars_restart_enddef
    if( URBAN_RESTART_OUTPUT .AND. TIME_DOURBAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call URBAN_vars_restart_enddef
    if( ATMOS_RESTART_OUTPUT .AND. TIME_DOATMOS_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call ATMOS_vars_restart_enddef

    ! write variabes to netCDF file
    if( OCEAN_RESTART_OUTPUT .AND. TIME_DOOCEAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call OCEAN_vars_restart_write
    if(  LAND_RESTART_OUTPUT .AND. TIME_DOLAND_restart  .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call  LAND_vars_restart_write
    if( URBAN_RESTART_OUTPUT .AND. TIME_DOURBAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call URBAN_vars_restart_write
    if( ATMOS_RESTART_OUTPUT .AND. TIME_DOATMOS_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call ATMOS_vars_restart_write

    ! clode the restart file
    if( OCEAN_RESTART_OUTPUT .AND. TIME_DOOCEAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call OCEAN_vars_restart_close
    if(  LAND_RESTART_OUTPUT .AND. TIME_DOLAND_restart  .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call  LAND_vars_restart_close
    if( URBAN_RESTART_OUTPUT .AND. TIME_DOURBAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call URBAN_vars_restart_close
    if( ATMOS_RESTART_OUTPUT .AND. TIME_DOATMOS_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 1 ) call ATMOS_vars_restart_close

    ! cread restart netCDF file(s)
    if( OCEAN_RESTART_OUTPUT .AND. TIME_DOOCEAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call OCEAN_vars_restart_create_add( RESTART_OUT_ADDITIONAL_BASENAME(3) )
    if(  LAND_RESTART_OUTPUT .AND. TIME_DOLAND_restart  .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call  LAND_vars_restart_create_add( RESTART_OUT_ADDITIONAL_BASENAME(3) )
    if( URBAN_RESTART_OUTPUT .AND. TIME_DOURBAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call URBAN_vars_restart_create_add( RESTART_OUT_ADDITIONAL_BASENAME(3) )
    if( ATMOS_RESTART_OUTPUT .AND. TIME_DOATMOS_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call ATMOS_vars_restart_create_add( RESTART_OUT_ADDITIONAL_BASENAME(3) )

    ! define metadata (dimensions, variables, attributes) in netCDF file
    if( OCEAN_RESTART_OUTPUT .AND. TIME_DOOCEAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call OCEAN_vars_restart_def_var
    if(  LAND_RESTART_OUTPUT .AND. TIME_DOLAND_restart  .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call  LAND_vars_restart_def_var
    if( URBAN_RESTART_OUTPUT .AND. TIME_DOURBAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call URBAN_vars_restart_def_var
    if( ATMOS_RESTART_OUTPUT .AND. TIME_DOATMOS_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call ATMOS_vars_restart_def_var

    ! exit define mode
    if( OCEAN_RESTART_OUTPUT .AND. TIME_DOOCEAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call OCEAN_vars_restart_enddef
    if(  LAND_RESTART_OUTPUT .AND. TIME_DOLAND_restart  .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call  LAND_vars_restart_enddef
    if( URBAN_RESTART_OUTPUT .AND. TIME_DOURBAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call URBAN_vars_restart_enddef
    if( ATMOS_RESTART_OUTPUT .AND. TIME_DOATMOS_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call ATMOS_vars_restart_enddef

    ! write variabes to netCDF file
    if( OCEAN_RESTART_OUTPUT .AND. TIME_DOOCEAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call OCEAN_vars_restart_write
    if(  LAND_RESTART_OUTPUT .AND. TIME_DOLAND_restart  .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call  LAND_vars_restart_write
    if( URBAN_RESTART_OUTPUT .AND. TIME_DOURBAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call URBAN_vars_restart_write
    if( ATMOS_RESTART_OUTPUT .AND. TIME_DOATMOS_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call ATMOS_vars_restart_write

    ! clode the restart file
    if( OCEAN_RESTART_OUTPUT .AND. TIME_DOOCEAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call OCEAN_vars_restart_close
    if(  LAND_RESTART_OUTPUT .AND. TIME_DOLAND_restart  .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call  LAND_vars_restart_close
    if( URBAN_RESTART_OUTPUT .AND. TIME_DOURBAN_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call URBAN_vars_restart_close
    if( ATMOS_RESTART_OUTPUT .AND. TIME_DOATMOS_restart .AND. RESTART_OUT_ADDITIONAL_COPIES > 2 ) call ATMOS_vars_restart_close
    !-- add by satoki for letkf to here
