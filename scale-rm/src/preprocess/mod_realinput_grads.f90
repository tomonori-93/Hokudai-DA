...
!             enddo
    ...
    if( IO_L ) write(IO_FID_LOG,*) '+++ ScaleLib/IO[realinput]/Categ[LandInputGrADS] check'  ! add satoki
    return
  ...
  subroutine read_grads_file_3d(  &
    ...
    irecl=nx*ny*4
!ORG(satoki)    call open_grads_file(io_fid, gfile, irecl)
    open(io_fid,                   &
         file   = trim(gfile),  &
         form   = 'unformatted',   &
         access = 'direct',        &
...
write(*,*) "check: "//trim(item), gdata(10,10,k), k, irec, it, totalrec, startrec
       if ( ierr /= 0 ) then
