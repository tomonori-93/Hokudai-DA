...
module mod_atmos_phy_ch_vars
  ...
  public :: ATMOS_PHY_CH_vars_restart_create_add  ! add by satoki
...
contains
  ...
  subroutine ATMOS_PHY_CH_vars_restart_open
    ...
    if( IO_L ) write(IO_FID_LOG,*)
    if( IO_L ) write(IO_FID_LOG,*) '*** Open restart file (ATMOS_PHY_CH) ***'
  ...
  subroutine ATMOS_PHY_CH_vars_restart_create_add( addfname )
    use scale_time, only: &
       TIME_gettimelabel
    use scale_fileio, only: &
       FILEIO_create
    ...
    if ( addfname /= '' ) then
       ...
       if( IO_L ) write(IO_FID_LOG,*)
       if( IO_L ) write(IO_FID_LOG,*) '*** Create restart file (ATMOS_PHY_CH) ***'
    ...
    return
  end subroutine ATMOS_PHY_CH_vars_restart_create_add
