...
!!
!<
!-- Add a new routine by satoki for letkf
!-------------------------------------------------------------------------------
...
module mod_atmos_vars
  ...
  public :: ATMOS_vars_restart_create_add  ! add by satoki
...
!OCL XFILL
  ...
  subroutine ATMOS_vars_restart_create_add( addfname )
    use scale_time, only: &
       TIME_gettimelabel
    use scale_fileio, only: &
       FILEIO_create
    use mod_atmos_admin, only: &
       ...
       ATMOS_sw_phy_cp
    use mod_atmos_dyn_vars, only: &
       ATMOS_DYN_vars_restart_create_add
    use mod_atmos_phy_mp_vars, only: &
       ATMOS_PHY_MP_vars_restart_create_add
    use mod_atmos_phy_ae_vars, only: &
       ATMOS_PHY_AE_vars_restart_create_add
    use mod_atmos_phy_ch_vars, only: &
       ATMOS_PHY_CH_vars_restart_create_add
    use mod_atmos_phy_rd_vars, only: &
       ATMOS_PHY_RD_vars_restart_create_add
    use mod_atmos_phy_sf_vars, only: &
       ATMOS_PHY_SF_vars_restart_create_add
    use mod_atmos_phy_tb_vars, only: &
       ATMOS_PHY_TB_vars_restart_create_add
    use mod_atmos_phy_cp_vars, only: &
       ATMOS_PHY_CP_vars_restart_create_add
#ifdef SDM
    use scale_atmos_phy_mp_sdm, only: &
       sd_rest_flg_out, &
       ATMOS_PHY_MP_sdm_restart_create_add
    use scale_time, only: &
       NOWSEC => TIME_NOWSEC
#endif
    implicit none
    ...
    if( sd_rest_flg_out ) then
       ...
       call ATMOS_PHY_MP_sdm_restart_create_add(NOWSEC)
    endif
#endif
    ...
    if( ATMOS_sw_dyn )    call ATMOS_DYN_vars_restart_create_add( addfname )
    if( ATMOS_sw_phy_mp ) call ATMOS_PHY_MP_vars_restart_create_add( addfname )
    if( ATMOS_sw_phy_ae ) call ATMOS_PHY_AE_vars_restart_create_add( addfname )
    if( ATMOS_sw_phy_ch ) call ATMOS_PHY_CH_vars_restart_create_add( addfname )
    if( ATMOS_sw_phy_rd ) call ATMOS_PHY_RD_vars_restart_create_add( addfname )
    if( ATMOS_sw_phy_sf ) call ATMOS_PHY_SF_vars_restart_create_add( addfname )
    if( ATMOS_sw_phy_tb ) call ATMOS_PHY_TB_vars_restart_create_add( addfname )
    if( ATMOS_sw_phy_cp ) call ATMOS_PHY_CP_vars_restart_create_add( addfname )

    return
  end subroutine ATMOS_vars_restart_create_add
