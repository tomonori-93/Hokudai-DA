...
module mod_atmos_phy_tb_vars
  ...
  public :: ATMOS_PHY_TB_vars_restart_create_add  ! add by satoki
...
contains
  ...
  subroutine ATMOS_PHY_TB_vars_setup
    ...
    if( IO_NML ) write(IO_FID_NML,nml=PARAM_ATMOS_PHY_TB_VARS)
...
!    if( IO_L ) write(IO_FID_LOG,*)
!    if( IO_L ) write(IO_FID_LOG,*) '*** Open restart file (ATMOS_PHY_TB) ***'
...
!    endif
  ...
  subroutine ATMOS_PHY_TB_vars_restart_create_add( addfname )
    use scale_time, only: &
       TIME_gettimelabel
    use scale_fileio, only: &
       FILEIO_create
...
!    endif
    ...
    return
  end subroutine ATMOS_PHY_TB_vars_restart_create_add
