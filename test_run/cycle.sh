      ...
      if [ "$CONF_MODE" = 'static' ]; then
        ...
        if ((enable_iter == 1 && nitmax > 1)); then
          for it in $(seq $nit); do
            ...
            if ((IO_ARB == 1)); then ##
              mpirunfmod ${nodestr} ./${stepexecname[$s]} $((NNODES*PPN)) ${stepexecname[$s]}_${conf_time}_${it}.conf log/${stepexecname[$s]}.NOUT_${conf_time}_${it} || exit $? &
#ORG satoki              mpirunf ${nodestr} ./${stepexecname[$s]} ${stepexecname[$s]}_${conf_time}_${it}.conf log/${stepexecname[$s]}.NOUT_${conf_time}_${it} || exit $? &
            else ##
              mpirunfmod ${nodestr} ./${stepexecname[$s]} $((NNODES*PPN)) ${stepexecname[$s]}_${conf_time}_${it}.conf log/${stepexecname[$s]}.NOUT_${conf_time}_${it} || exit $?
#ORG satoki              mpirunf ${nodestr} ./${stepexecname[$s]} ${stepexecname[$s]}_${conf_time}_${it}.conf log/${stepexecname[$s]}.NOUT_${conf_time}_${it} || exit $?
            fi ##
          ...
          if ((IO_ARB == 1)); then ##
            if ((s == 5)); then ##
              mpirunfmod ${nodestr} ./${stepexecname[$s]} $((NNODES*PPN)) ${stepexecname[$s]}_${conf_time}.conf log/${stepexecname[$s]}.NOUT_${conf_time} \
#ORG satoki              mpirunf ${nodestr} ./${stepexecname[$s]} ${stepexecname[$s]}_${conf_time}.conf log/${stepexecname[$s]}.NOUT_${conf_time} \
                      "$SCRP_DIR/sleep.sh" || exit $? &
            else ##
              mpirunfmod ${nodestr} ./${stepexecname[$s]} $((NNODES*PPN)) ${stepexecname[$s]}_${conf_time}.conf log/${stepexecname[$s]}.NOUT_${conf_time} || exit $? &
#ORG satoki              mpirunf ${nodestr} ./${stepexecname[$s]} ${stepexecname[$s]}_${conf_time}.conf log/${stepexecname[$s]}.NOUT_${conf_time} || exit $? &
            fi ##
          else ##
            mpirunfmod ${nodestr} ./${stepexecname[$s]} $((NNODES*PPN)) ${stepexecname[$s]}_${conf_time}.conf log/${stepexecname[$s]}.NOUT_${conf_time} || exit $?
#ORG satoki            mpirunf ${nodestr} ./${stepexecname[$s]} ${stepexecname[$s]}_${conf_time}.conf log/${stepexecname[$s]}.NOUT_${conf_time} || exit $?
        ...
        if ((enable_iter == 1)); then
          for it in $(seq $nit); do
            ...
            if ((IO_ARB == 1)); then ##
              mpirunfmod $nodestr $execpath $((NNODES*PPN)) ${execpath}.conf "${stdout_dir}/NOUT-${it}" "$SCRP_DIR/${job}_step.sh" "$time" $loop $it || exit $? &
#ORG satoki              mpirunf $nodestr $execpath ${execpath}.conf "${stdout_dir}/NOUT-${it}" "$SCRP_DIR/${job}_step.sh" "$time" $loop $it || exit $? &
            else ##
              mpirunfmod $nodestr $execpath $((NNODES*PPN)) ${execpath}.conf "${stdout_dir}/NOUT-${it}" "$SCRP_DIR/${job}_step.sh" "$time" $loop $it || exit $?
#ORG satoki              mpirunf $nodestr $execpath ${execpath}.conf "${stdout_dir}/NOUT-${it}" "$SCRP_DIR/${job}_step.sh" "$time" $loop $it || exit $?
            fi ##
          ...
          if ((IO_ARB == 1)); then ##                                 
            mpirunfmod $nodestr $execpath $((NNODES*PPN)) ${execpath}.conf "${stdout_dir}/NOUT" "$SCRP_DIR/${job}_step.sh" "$time" "$loop" || exit $? &
#ORG satoki            mpirunf $nodestr $execpath ${execpath}.conf "${stdout_dir}/NOUT" "$SCRP_DIR/${job}_step.sh" "$time" "$loop" || exit $? &
          else ##
            mpirunfmod $nodestr $execpath $((NNODES*PPN)) ${execpath}.conf "${stdout_dir}/NOUT" "$SCRP_DIR/${job}_step.sh" "$time" "$loop" || exit $?
#ORG satoki            mpirunf $nodestr $execpath ${execpath}.conf "${stdout_dir}/NOUT" "$SCRP_DIR/${job}_step.sh" "$time" "$loop" || exit $?
          fi ##
