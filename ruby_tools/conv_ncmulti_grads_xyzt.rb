...
outfile = ARGV[2]
#outfile = "hist.dmp_20180927-120000.bin"
aval = [ "U", "V", "W", "PRES", "PT" ]  # ens_mean variable
axname = [ "x", "y", "z", "time" ]
...
for i in 1..ens_num.to_i
   ...
   for j in 0..aval.size-1
      ...
      time_units = u_memv.axis(axname[3]).pos.get_att("units")
      time_units = time_units.gsub(time_attr_replace[0],time_attr_replace[1])
#p u_memv.axis(axname[2]).pos.val

      addax2_var = VArray.new( u_memv.axis(axname[3]).pos.val, {"long_name"=>u_memv.axis(axname[3]).pos.get_att("long_name"), "units" => time_units}, axname[3])
...
#p tax.pos.get_att("units")
      ...
      for l in 0..tax.length-1
         u_var[0..xax.length-1,0..yax.length-1,0..zax.length-1,l] = u_memv.val[0..xax.length-1,0..yax.length-1,0..zax.length-1,l]
      end
