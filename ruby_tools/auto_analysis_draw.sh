...
obs_file=("../obs/T1824.2km.NCEP.03.dmp.vt.united.bin_SCALE_r100.nc" "../obs/T1824.2km.NCEP.03.dmp.vr.united.bin_SCALE_r100.nc")  # 観測に用いた true run (anl_var の vt, vr に順番を合わせる)
anl_day="201809"  # 解析月 yyyymm
anl_time=("26-00" "26-12" "27-00" "27-12")  # 解析時刻
tru_time=("00388800" "00432000" "00475200" "00518400")  # 解析時刻 8 桁 (true run の経過秒数基準)
...
truetc_file="T1824.2km.NCEP.03.geography.CReSStoSCALE.tcenter"  # true run の tcenter ファイルをアンサンブルメンバーの格子点で置き換えた (calc_nearest_latlon 適用後の tcenter ファイル)
...
rrange="0.0:200000.0"  # 描画する距離範囲の設定
zrange="0.0:20000.0"  # 描画する高度範囲の設定
...
for i in 0 1 2; do  # 0: vt, 1: vr, 2: w
   for j in ${anl_time[@]}; do
      gpview --anim t --wsn=2 --clrmap=4 --levels ${var_levels[$i]} --patterns ${var_pattns[$i]} ${wkdir}/.${drw_var[$i]}/hist.dmp_${anl_day}${j}0000.bin.dat.nc@${drw_var[$i]},r=${rrange},z=${zrange}
...
for i in 0 1 3; do  # 0: vt, 1: vr, 3: pt_pert
   for j in ${anl_time[@]}; do
      gpview --wsn=2 --clrmap=4 --levels ${var_levels[$i]} --patterns ${var_pattns[$i]} ${wkdir}/.${drw_var[$i]}/hist.dmp_${anl_day}${j}0000.bin.dat_Moveave.nc@${drw_var[$i]%_pert},t=^0,r=${rrange},z=${zrange}
...
for i in 0 1; do  # 0: vt, 1: vr
   for j in ${anl_time[@]}; do
      gpview --wsn=2 --clrmap=4 --levels ${dvar_levels[$i]} --patterns ${dvar_pattns[$i]} ${wkdir}/.${anl_var[$i]}/hist.dmp_${anl_day}${j}0000.bin.dat_Moveave_obsdiff.nc@d${anl_var[$i]},r=${rrange},z=${zrange}
...
if [ ${wkdir} != ${ref_dir} ]; then  # フリーランデータの処理時はこの操作はしない
   for i in 0 1 3; do  # 0: vt, 1: vr, 3: pt
      for j in ${anl_time[@]}; do
         gpview --wsn=2 --clrmap=4 --levels ${dvar_levels[$i]} --patterns ${dvar_pattns[$i]} ${wkdir}/.${anl_var[$i]}/hist.dmp_${anl_day}${j}0000.bin.dat_Moveave_diff.nc@d${anl_var[$i]},t=^0,r=${rrange},z=${zrange}
...
for j in "${!anl_time[@]}"; do  # "!" で anl_var の添え字のリストになっているので, j には 0~ 整数が入る
   if [ $j != 0 ]; then
      for i in 0 1 3; do  # 0: vt, 1: vr, 3: pt_pert
         for k in anal gues; do
            gpview --wsn=2 --clrmap=4 --levels ${var_levels[$i]} --patterns ${var_pattns[$i]} ${wkdir}/mean_ruby/mean_${k}_${anl_day}${anl_time[$j]}0000.000.bin.${drw_var[$i]}.nc@${drw_var[$i]%_pert},t=^0,r=${rrange},z=${zrange}
...
for j in "${!anl_time[@]}"; do  # "!" で anl_var の添え字のリストになっているので, j には 0~ 整数が入る
   if [ $j != 0 ]; then
      for i in 0 1 3; do  # 0: vt, 1: vr, 3: pt
         gpview --wsn=2 --clrmap=4 --levels ${divar_levels[$i]} --patterns ${dvar_pattns[$i]} ${wkdir}/mean_ruby/mean_anal_${anl_day}${anl_time[$j]}0000.000.bin.${anl_var[$i]}_diff.nc@d${anl_var[$i]},t=^0,r=${rrange},z=${zrange}
...
for j in "${!anl_time[@]}"; do
   if [ $j != 0 ]; then
      for i in 0 1; do  # 0: vt, 1: vr
         gpview --wsn=2 --clrmap=4 --levels ${dvar_levels[$i]} --patterns ${dvar_pattns[$i]} ${wkdir}/mean_ruby/mean_anal_${anl_day}${anl_time[$j]}0000.000.bin.${anl_var[$i]}_obsdiff.nc@d${anl_var[$i]},r=${rrange},z=${zrange}
...
for j in "${!tru_time[@]}"; do
   for i in 0 1; do  # 0: vt, 1: vr
      gpview --wsn=2 --clrmap=4 --levels ${var_levels[$i]} --patterns ${var_pattns[$i]} ${obs_file[$i]}@${drw_var[$i]},r=${rrange},z=${zrange},t=${tru_time[$j]}
