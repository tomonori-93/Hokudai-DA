...
nr_dir=../obs_UV/      # pwd から見た Nature run (nc) データのディレクトリ相対パス
wk_nr_dir=../../../obs_UV/   # wkdir から見た Nature run (nc) データのディレクトリ相対パス
nr_file=("T1824.2km.NCEP.03.dmp.vt.united.bin_SCALE_r100.nc" "T1824.2km.NCEP.03.dmp.vr.united.bin_SCALE_r100.nc")  # Nature run のファイル名
...
bst_geo_orig=T1824.2km.NCEP.03.geography.united.bin_SCALE
# (上): Nature run の tcenter/asuca_z データ (テキスト) のコピー元ファイル名
bst_geo_name=bst_T1824_tot.dat
# (上): bst_geo_orig のコピー先ファイル名
tint=("26-00" "26-06" "26-12" "26-18" "27-00" "27-06" "27-12" "27-18" "28-00")  # 解析処理を行う時刻 (dd-hh)
tint_sec=("00388800" "00410400" "00432000" "00453600" "00475200" "00496800" "00518400" "00540000" "00561600")  # tint に対応する sec
#-- 描画に関すること
...
for k in anal gues; do
   ...
   for var in rot div; do
      ...
      for j in ${!tint[@]}; do
         ...
         dcl_nml_change.rb ${nml} title_name "'${var} @ 1 km (x10\^{-3} s\^{-1})'"
...
for var in rot div; do
   ...
   for j in ${!tint[@]}; do
      ...
      dcl_nml_change.rb ${nml} title_name "'Δ${var} @ 1 km (x10\^{-3} s\^{-1})'"
      dcl_nml_change.rb ${nml} ininame "${tint_sec[$j]}"
      dcl_nml_change.rb ${nml} grdname "'${wkdir}/hist.geo_${ininame_yyyymm}${ininame_dd}-${ininame_hh}0000.bin.dat'"
      ./dcl_sfc_map < ${nml}
