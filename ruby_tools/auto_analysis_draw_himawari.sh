...
#-- 実験結果ディレクトリの指定
#exp_name="48h"
exp_name="1h_eye_A08_himawari"
res_dir="../exp/testcase_${exp_name}/"
wkdir="1_output/${exp_name}/"
rev_dir="../../"  # wkdir からみた cur_dir 相対パス
ref_dir="1_output/48h/"  # フリーランのディレクトリ
obs_file=("../obs_identicaltwin/UV_.vt.grd.nc" "../obs_identicaltwin/UV_.vr.grd.nc")  # 観測に用いた true run (このスクリプトは OSSE 解析しないので, このデータはダミー)
...
truetc_file="bst_T1824_himawari.dat.tcenter"  # true run の tcenter ファイルをアンサンブルメンバーの格子点で置き換えた (calc_nearest_latlon 適用後の tcenter ファイル)
truetc_orgdir="1_output/48h/"  # truetc_file のオリジナルデータの所在ディレクトリ
#-- 以下は描画関連の変数設定
var_levels=("0.0,10.0,20.0,30.0,40.0,50.0,60.0" \
