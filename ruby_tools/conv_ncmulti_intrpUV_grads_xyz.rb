...
#ifile = "../exp/testcase_1h_eye_A02/mean_ruby/mean_anal_20180926-120000.pe00"  # ORG
ifile = ARGV[0]
ofoot = ".bin"  #<- .pe000000.nc を .bin に変換して出力
ens_num = 30
####### setting variables #########

ilist = `ls #{ifile}*.nc`.split("\n")
ofile = ilist[0].gsub("pe000000.nc","bin")
...
dens = GPhys::IO.open_multi(ilist,rhoc)
...
# Interpolation for u
uxrho = GPhys::IO.open_multi(ilist,ivals[0]).copy  # copy により, メモリ上に NetCDF データを読み込み (gphys オブジェクト uxrho への書き込みを可能にする.)
...
# Interpolation for v
vxrho = GPhys::IO.open_multi(ilist,ivals[1]).copy
# uxrho と同様の置き換え
...
ptxrho = GPhys::IO.open_multi(ilist,ivals[2]).copy
pt_itp = ptxrho / dens
pt_itp.name = ovals[2]
